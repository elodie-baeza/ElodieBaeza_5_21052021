...
const recipesClean = clean();

function formatString(str) {
    return str.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().trim();
}

function tableOfWords(sentence) {
    var tableOfWords = [];
...
function research(str, result) {
    const recipesResultList = new Set();
    const arrayMainInput = tableOfWords(str);

    result.forEach(recipe => {
        if (arrayMainInput.every(x => recipe.name.includes(x) || recipe.description.includes(x))) {
            recipesResultList.add(recipesClean[recipe.id-1]);
        }

        recipe.ingredients.forEach(ingredient => {
            if (arrayMainInput.every(x => ingredient.includes(x))) {
                recipesResultList.add(recipesClean[recipe.id-1]);
            }
        });
});
